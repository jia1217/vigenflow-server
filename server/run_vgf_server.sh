...
NPU_FILES_PATH="/home/kelsey/NPU_projects/NPU_repo/deb_lib/vigenflow/npu_files"
PORT="11281"
EXE_BASE_DIR="/home/kelsey/NPU_projects/NPU_repo/deb_lib/vigenflow/exe_models"
EXE_PATH=""
...
# Defaults download z-image-turbo-Q4_1.gguf from Hugging Face when packed weights are missing.
GGUF_PATH=""
GGUF_REPO="unsloth/Z-Image-Turbo-GGUF"
GGUF_FILE="z-image-turbo-Q4_1.gguf"
GGUF_REVISION="main"
GGUF_HF_TOKEN=""
FORCE_GGUF_DOWNLOAD="false"

# BF16 options for z-image-turbo-BF16. Download/export is implemented in C++.
...
VGF_ARGS=("$MODEL_NAME" -w "$WEIGHTS_PATH" -n "$NPU_FILES_PATH" -p "$PORT")
...
exec "$SCRIPT_DIR/vgf-serve" "${VGF_ARGS[@]}" "${GGUF_ARGS[@]}" "${Z_IMAGE_BF16_ARGS[@]}" "${FLUX_KLEIN_BF16_ARGS[@]}" "${LORA_ARGS[@]}" "$@"
