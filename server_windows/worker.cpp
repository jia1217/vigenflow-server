...
// 2. Revert to the standard Boost Process header
// #include <boost/process.hpp>
#include <atomic>
#include <algorithm>

#include <boost/process/v1/child.hpp>      // <-- Added v1/
#include <boost/process/v1/args.hpp>       // <-- Added v1/
#include <boost/process/v1/io.hpp>
#include <boost/process/v1/start_dir.hpp>  // <-- Added v1/
#include <chrono>
#include <cctype>
#include <filesystem>
#include <fstream>
...
bool resolve_z_image_worker_paths(const std::string& raw_target, WorkerPaths& paths) {
    ...
    if (is_z_image_q41_base_model_id(raw_target)) {
        ...
        paths.exe = config::exe_base_dir + "/" + z_image_npu_target + "/q41/host.exe";
        return true;
    }

    if (is_z_image_q41_lora_target(raw_target)) {
        paths.npu_files = config::npu_base_dir + "/" + z_image_npu_target;
        paths.exe = config::exe_base_dir + "/" + z_image_npu_target + "/lora_add_q41/host.exe";
        return true;
    }

    if (is_z_image_bf16_base_model_id(raw_target) || is_z_image_bf16_lora_target(raw_target)) {
        paths.npu_files = config::npu_base_dir + "/" + z_image_npu_target;
        paths.exe = config::exe_base_dir + "/" + z_image_npu_target + "/bf16/host.exe";
        return true;
    }
...
WorkerPaths resolve_worker_paths(const std::string& raw_target) {
    ...
    if (raw_target == "flux.2-klein-4B-edit" || raw_target == "flux.2-klein-4B-edit-lora") {
        ...
        paths.exe = config::exe_base_dir + "/flux.2-klein-4B-edit/host.exe";
    } else if (raw_target == "flux.2-klein-4B-lora") {
        paths.npu_files = config::npu_base_dir + "/flux.2-klein-4B";
        paths.exe = config::exe_base_dir + "/flux.2-klein-4B/host.exe";
        ...
        paths.exe = config::exe_base_dir + "/" + raw_target + "/host.exe";
    }
